...
Page({
  ...
  bindGetUserInfo: function (e) {
    if (e.detail.userInfo) {
      ...
      wx.showModal({
        title: '拒绝授权',
        content: '您拒绝了微信授权，我们无法为您提供相应服务，请重新点击登录',
        showCancel: false,
        confirmText: '确定',
        success: function (res) {
  ...
  formSubmit: function(e) {
    ...
    if(again){
      db.collection('score').doc(this.data.list._id).set({
        data: formData,
        success: function (res){
          console.log(res);
          wx.showModal({
            title: '更新成功',
            content: '成绩数据已成功同步至云端数据库，点击确定返回主页。',
            showCancel: false,
            confirmText: '确定',
            success: function (res) {
              wx.switchTab({
      ...
      score.add({
        ...
        success: function (res){
          ...
          wx.showModal({
            title: '提交成功',
            content: '成绩数据已成功提交至云端数据库，点击确定返回主页。',
            showCancel: false,
            confirmText: '确定',
            success: function (res) {
              wx.navigateBack({
  ...
  delete: function(res){
    const db = wx.cloud.database();
    db.collection('score').doc(this.data.list._id).remove({
      success:function (res){
        console.log(res);
        wx.showModal({
          title: '删除成功',
